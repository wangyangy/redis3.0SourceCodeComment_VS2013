...
//创建共享对象的函数,一些常用的变量创建出来
void createSharedObjects(void) {
    int j;
	//REDIS_STRING是一个宏定义,值为0.
...
void initServerConfig(void) {
    int j;
	//server.runid是一个char类型的数组,REDIS_RUN_ID_SIZE+1是这个数组的长度
...
void initServer(void) {
	...
	//创建客户端,这里客户端用list连接,会调用adlist.c文件中的函数,相当于创建一个list的哨兵结构,head和tail都为null
    ...
    replicationScriptCacheInit();// 初始化复制脚本缓存
	scriptingInit(); // 初始化lua脚本系统
    slowlogInit();// 初始化慢查询
	latencyMonitorInit();  // 初始化延迟诊断的功能
    bioInit();// 初始化后台IO
}
...
void call(redisClient *c, int flags) {
    ...
    dirty = server.dirty;// 备份脏键数
    start = ustime();// 获取执行命令的开始时间
    c->cmd->proc(c);// 执行命令
	duration = ustime() - start; // 命令的执行时间
    dirty = server.dirty-dirty;// 命令修改的键的个数
    if (dirty < 0) dirty = 0;
    ...
    if (server.also_propagate.numops) {
		...
		redisOpArrayFree(&server.also_propagate); // 释放Redis操作数组
    }
    server.stat_numcommands++;// 命令执行的次数加1
}
...
int processCommand(redisClient *c) {
	...
	*/
    ...
    {
        queueMultiCommand(c);// 除了上述的四个命令，其他的命令添加到事务队列中
        addReply(c,shared.queued);
	// 执行普通的命令
    } else {
        call(c,REDIS_CALL_FULL);
        c->woff = server.master_repl_offset;// 保存写全局的复制偏移量
		if (listLength(server.ready_keys)) // 如果因为BLPOP而阻塞的命令已经准备好，则处理client的阻塞状态
...
//取消注释就能使用了,这是为了测试别的文件方便使用的,参考的sds.c文件中的写法,还是不太会使用
//#define REDIS_TEST_MAIN
...
int main(int argc, char **argv) {
	...
	//初始化redisSever,该函数就是把redisServer结构体中的内容通过redis.h中定义的常亮进行初始化
    initServerConfig();
    ...
    redisSetProcTitle(argv[0]);  //为服务器进程设置标题
    redisAsciiArt();             //打印redis的Logo
    checkTcpBacklogSettings();   //检查backlog队列
	//如果不是哨兵模式
    if (!server.sentinel_mode) {
